...
The first really important thing I understood is that a \keyword{GPU} is driven by a host \keyword{CPU}. In the case of \bcm, the \vc{} is driven by an \cpu{}(ARM Cortex A53). Furthermore, the \vc{} and the \cpu{} share the same \keyword{RAM} memory:

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.4\textwidth]{sharedRAM}
\caption{\vc{} and \cpu{} Shared RAM memory}
\label{sharedRAM}
\end{figure}
\FloatBarrier

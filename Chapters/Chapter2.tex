...
\emph{Raspberry Pi Playground} \parencite{refRpiPlayground} is the first project I focused on to better understand the \vc. It is associated with a \enquote{\code{github}} repository, here : \url{//github.com/elorimer/rpi-playground}

It is the most important project I studied during my internship because it contains a \enquote{\code{helloworld}} program that helped me to understand the preliminary basis of the \vc{} architecture and above all how to use this \keyword{GPU}. I detail this program because all the rest of my internship is based on it. I followed every step of this project permanantly reading the \vc{} documention \parencite{refVC}.


\subsection{\code{helloworld} Program}
...
The second \keyword{key thing} is that they share the same \keyword{RAM} memory as Appendice~\ref{AppendixB} taken from the \bcm{} documentation \parencite{refBCM} shows:

%\begin{figure}[!htbp]
%\centering
%\includegraphics[width=0.9\textwidth]{BCM2837memory}
%\caption{\vc{} and \cpu{} Shared RAM}
%\label{sharedFigure}
%\end{figure}
%\FloatBarrier

As a consequence, in the case of a \keyword{GPU} programing, a \code{helloworld} is not a trivial exercice because a lot of parts are involved. To run \code{code} on \vc, we must write a \cpu{} program (in \keyword{C} language) that will:
\begin{itemize}
...
Within the \code{rpi-playground} project, \code{helloworld} program will:
...
Appendice~\ref{AppendixC} taken from \parencite{refVC} shows an architecture overview of the \vc. The \keyword{GPU} parts involved \code{helloworld} program are highlighted with \textcolor{blue}{blue} rectangle:

%\begin{figure}[!htbp]
%	\centering
%	\includegraphics[width=0.9\textwidth]{architectureOverview}
...
\begin{itemize}
	\item \keyword{AXI ARB} -- bus that connects \vc{} to shared RAM
...
\cpu{} and \vc{} communicate through \keyword{AXI ARB} bus. Within \code{helloworld} program, we will use \cpu{} to:
\begin{itemize}
	...
	\item execute code using \keyword{QPU}
	\item give back results to \cpu{} through \keyword{VDM}
\end{itemize}


\subsubsection{Quad Processor Unit}

\qpu{} is the \vc{} internal processor. It is the \keyword{key component}. Appendice~\ref{AppendixD} taken from \parencite{refVC} shows the pipeline:
...
\subsection{\code{rpi-playground} Architecture}

\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.4\textwidth,scale=0.2]{rpiPlaygroundArchitecture}
	\caption{\code{rpi-playground} Architecture}
...
\item \keyword{helloworld} directory contains:
	\begin{itemize}
		\item \file{helloworld.asm} -- \keyword{code-to-execute} on the \vc
		\item \file{driver.c} -- code that \keyword{drives} the \vc
	\end{itemize}
\end{itemize}
...
\file{assemble.cpp} is the \keyword{assembly parser} written by Eric \textsc{Lorimer} the author of \file{rpi-playground} project. This code translates instuctions written in the \file{helloworld.asm} file of the \keyword{helloworld} directory (Figure~\ref{rpiPlaygroundFigure}) into a binary file (\file{helloworld.bin}) that the \vc{} will understand.

Figure~\ref{VCinstructionsFigure} taken from \parencite{refVC} shows the \enquote{\code{add operations}} of the \vc{}. This \keyword{GPU} has also \enquote{\code{mul}}, \enquote{\code{load}}, \enquote{\code{branch}} or \enquote{\code{semaphore}} instructions.

\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.9\textwidth]{VCinstructions}
	\caption{Part of \vc{} instructions set}
	\label{VCinstructionsFigure}
\end{figure}
\FloatBarrier

In the \file{assemble.cpp} file, these instructions are declared with the following \keyword{C++} code:
...
In these files we define a \code{MACRO} function, \code{BUS\_TO\_PHYS()}, to convert \keyword{VC CPU Bus Addresses} into \keyword{ARM Physical Addresses} (Appendice~\ref{AppendixB}).

Then we use \code{MACROS} to define \keyword{VC CPU Bus Addresses} of \vc{} peripherals.

Finally, we declare \code{int gpu\_fft\_get\_host\_info()} to get \rasp{} information.
...
\file{helloworld.asm} is the \keyword{code-to-execute} by the \vc. It is written in \keyword{assembly language} using the \vc{}'s instructions set.

\code{\# Load the value we want to add to the input into a register}\\
\code{ldi ra1, 0x1234}\\
...
\code{\# Configure the VPM for writing}\\
\code{ldi rb49, 0xa0}\\
...
\code{\# Add the input value (first uniform - rb32) and the register with the hard-coded constant into the VPM.}\\
\code{add rb48, ra1, rb32; nop}\\
-- You can find the QPU register address map on pages 37-38 where it shows writing to rb49 is VPMVCD\_WR\_SETUP. The VPM interface is documented in section 7 and table 32 describes the format for this register.
-- 0xa00 means to write with no stride, starting at VPM location (0, 0) with horizontal format and 32-bit elements. See VC4 documentation pages 54-55.\\~\\
%----------------------------------------------------------------------------------------

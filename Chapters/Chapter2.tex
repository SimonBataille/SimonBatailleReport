...
\begin{itemize}
	\item initialize and allocate \ram{} in the \keyword{ARM Virtual Adresses} (Appendice~\ref{AppendixB})
...
\lstset{style=CStyle,caption={init \mail{} interface}}
\begin{lstlisting}
    int mb = mbox_open();
    if (qpu_enable(mb, 1))
    {
...
\begin{itemize}
	\item We use two \keyword{uniforms} to transfer:
		...
		\end{itemize}
		from the host \cpu{} to the \emph{QPU 0,0} through the \mail{} interface (\code{execute\_qpu()} function in \file{driver.c}). These 2 uniforms will be load into \emph{QUC : Uniforms Cache}.
	\item We also use \mail{} interface to tranfer \code{code-to-execute} to the \keyword{GPU}. \code{code-to-execute} is then load into \emph{QIC : Icache (Instructions cache)}.
	...
	\item The result (16-word vector) is write back from \keyword{VPM} to \ram{} memory through \keyword{DMA} transfer.
\end{itemize}

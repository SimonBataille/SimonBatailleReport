...
For instance, matrices which are stored in the shared RAM can be accessed knowing the pointer of the first element. I also noticed that matrices are stored in \emph{row-major} order. That means row values are contiguous in shared RAM memory. This is a \keyword{key point} to know because frames from the \rasp{} camera are stored as \option{float-matrices} in memory and to compute the \keyword{optical flow} we have to access data from those frames.

...
%\subsection{Driver.c}
%\lstinputlisting[lastline]{grad.c}
...
The second \keyword{key thing} is that they share the same \keyword{RAM} memory as figure~\ref{sharedFigure} taken from the \bcm{} documentation\parencite{refBCM} shows:
...
So, within the \code{rpi-playground} project, the \code{helloworld} program will:
...
Figure~\ref{archFigure} taken from \parencite{refVC} shows an architecture overview of the \vc. The \keyword{GPU} parts involved \code{helloworld} program are highlighted with \textcolor{blue}{blue} rectangle.
...
\begin{itemize}
\item initialize and allocate shared RAM in the \keyword{ARM Virtual Adresses} (Figure~\ref{sharedFigure})
...
\qpu{} is the \vc{} internal processor. It is the \keyword{key component}. Figure~\ref{qpuFigure} taken from \parencite{refVC} shows the pipeline:
...
\item \keyword{assembler} directory contains:
	\begin{itemize}
		\item \file{assemble.cpp} -- the \keyword{assembly parser} of \vc{} instructions set
	\end{itemize}
\item \keyword{helloworld} directory contains:
	\begin{itemize}
		\item \file{helloworld.asm} -- the \keyword{code-to-execute} on the \vc
		\item \file{driver.c} -- the code that \keyword{drive} the \vc
	\end{itemize}
\end{itemize}
...
\lstset{language=C++,caption={Declaration of \enquote{\code{add operations}} in \file{assemble.cpp}},captionpos=b,label=addLabel}
\begin{lstlisting}
...
This \file{Makefile} contains only one instruction. So running the \code{make} command inside the \keyword{assembler} directory generates \file{qpu-assembler} \keyword{executable}.
\vspace{5 mm}

Then, this \keyword{executable} is used in the \keyword{helloworld} directory to generate \file{helloworld.bin}:

\lstset{language=make,caption={qpu-assembler run on helloworld.asm},captionpos=b,label=executeLabel}
\begin{lstlisting}

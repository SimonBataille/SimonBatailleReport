...
It is the most important project I studied during my internship because it contains a \enquote{\file{helloworld}} program that helped me to understand the preliminary basis of the \vc{} architecture and above all how to use this \keyword{GPU}. I detail this program because all the rest of my internship is based on it. I followed every step of this project permanantly reading the \vc{} documention \parencite{refVC}.
...
As a consequence, in the case of a \keyword{GPU} programing, a \file{helloworld} is not a trivial exercice because a lot of parts are involved. To run \code{code} on \vc, we must write a \cpu{} program (in \keyword{C} language) that will:
\begin{itemize}
...
Appendice~\ref{AppendixC} taken from \parencite{refVC} shows an architecture overview of the \vc. The \keyword{GPU} parts involved \file{helloworld} program are highlighted with \textcolor{blue}{blue} rectangle:
...
\file{helloworld.asm} is the \code{code-to-execute} by the \vc. It is written in \keyword{assembly language} using the \vc{}'s instructions set.

...
\file{gradX.asm} is reproduced appendice~\ref{AppendixG}. The \keyword{key point} is that all \qpu{}s store the same \uni{}s at the beginning of the execution, so every \file{.asm} file start the same way:
...
All these \file{.asm} files form every step of the \flow{} algorithm on the \vc{}. To write them I have been massively based on the \vc{} architecture docuentation \parencite{refVC} and \emph{pi-gemm} project.

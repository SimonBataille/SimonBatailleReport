...
\file{grad.h} and \file{grad.c} - Appendice ~\ref{AppendixG} - contain the \code{code} that \emph{drives} the \vc. To write these files I followed the same steps as in~\ref{GPUconclusion} section.
...
\begin{itemize}
	...
	\item \ram{} layout - \code{struct grad\_MemLayout} - that the \code{compute\_lk\_gpu()} needs to run well - \option{lines 47 to 155}
...
\begin{itemize}
	...
	\item \option{lines 97 to 155} are all the \ram{} memory spaces for \qpu{}s to store results from \code{code-to-execute}s
\end{itemize}
...
\begin{itemize}
	...
	\item copy code (\file{.bin} file) and input data ($\textcolor{blue}{firstFrame}$, $\textcolor{red}{secondFrame}$ and $features\_static$) in \ram
	\item send \code{code-to-execute} \keyword{pointers} and \uni{}s to \qpu{}s
	\item get back the results from the \ram
\end{itemize}

With this driver, I managed to make the \api{} modular. Namely it is possible to execute different codes from different \file{.asm} files on the \vc.
To achieve that the \keyword{key points} are:

\begin{itemize}
	\item \option{lines 117 to 201} - within \code{grad\_setVCptrs()} function, we fill \code{grad\_VCptrsArray[]} with all the adresses defined in \code{struct grad\_MemLayout}. These addresses are located in \emph{VC CPU Bus Adresses}.
	\item \option{lines 226 to 366} - within \code{grad\_init()} function, we load all \file{.bin} files at the corresponding address in the \emph{VC CPU Bus adderesses}.
	\item pass different \code{code-to-execute} \keyword{pointers} to \code{execute\_qpu()} to select which one to execute on the \vc. This is done by changing \code{arm\_map->msg[i][1]} value.
\end{itemize}
...
\file{.asm} files contain \code{code-to-execute} on \vc. These files are written in \option{assembly language}. Figure~\ref{optFlowFig} shows all these files:
\begin{itemize}
	...
	\item \file{dPosXY.asm} computes 30 $(d_{x},d_{y})$ - \flow{} between \textcolor{blue}{firstFrame} and \textcolor{red}{secondFrame}
\end{itemize}

All these \file{.asm} files start the same way. The \keyword{key point} is that \qpu{}s store all the same \uni{}s at the beginning of the execution:
...
\begin{itemize}
	...
	\item addresses for \qpu{}s to store results from \code{code-to-execute}s
	\item all these addresses are \emph{VC CPU Bus addresses}
\end{itemize}

Listing~\ref{uniram} shows how \cpu{} stores those addresses in \ram{} and then pass them to \vc{} in \code{arm\_msg[i][0]}:

...
\begin{itemize}
	\item \keyword{prev\_gray} -- \textcolor{blue}{firstFrame}
	\item \keyword{gray} -- \textcolor{red}{secondFrame}
...
\begin{itemize}
	\item \option{pre-processing} -- Section ~\ref{parameters}
		\begin{itemize}
			\item convert \file{cv::Mat} from camera to a float-pointer on (240*360) frame values 
			\item convert \file{cv::Mat} from camera to a float-pointer on (240*360) frame values 
			\item convert \file{cv::Mat} from camera to a float-pointer on (240*360) frame values 
		\end{itemize}
	\item \option{GPU}

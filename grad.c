...
void grad_setVCptrs()
{
    grad_VCptrsArray[0] = vc_ptr + offsetof(struct grad_MemLayout, gradXcode); //vc_gradXcode
    grad_VCptrsArray[1] = vc_ptr + offsetof(struct grad_MemLayout, gradYcode); //vc_gradYcode
    grad_VCptrsArray[2] = vc_ptr + offsetof(struct grad_MemLayout, gradXYcode); //vc_gradXYcode
    grad_VCptrsArray[3] = vc_ptr + offsetof(struct grad_MemLayout, xtrBIP2code); //vc_xtrBIP2code
    grad_VCptrsArray[4] = vc_ptr + offsetof(struct grad_MemLayout, computeBIP2code); //vc_computeBIP2code
    grad_VCptrsArray[5] = vc_ptr + offsetof(struct grad_MemLayout, dPosXYcode); //vc_dPosXYcode


    grad_VCptrsArray[6] = vc_ptr + offsetof(struct grad_MemLayout, uniforms); //vc_uniforms


    grad_VCptrsArray[7] = vc_ptr + offsetof(struct grad_MemLayout, msg); //vc_msg


    grad_VCptrsArray[8] = vc_ptr + offsetof(struct grad_MemLayout, firstFrameShared); //vc_firstFrameShared
    grad_VCptrsArray[9] = vc_ptr + offsetof(struct grad_MemLayout, secondFrameShared); //vc_secondFrameShared
    grad_VCptrsArray[10] = vc_ptr + offsetof(struct grad_MemLayout, featureXYshared); //vc_featureXYshared


    grad_VCptrsArray[11] = vc_ptr + offsetof(struct grad_MemLayout, gradXshared); //vc_gradXshared
    grad_VCptrsArray[12] = vc_ptr + offsetof(struct grad_MemLayout, gXXshared); //vc_gXXshared


    grad_VCptrsArray[13] = vc_ptr + offsetof(struct grad_MemLayout, gradYshared); //vc_gradYshared
    grad_VCptrsArray[14] = vc_ptr + offsetof(struct grad_MemLayout, gYYshared); //vc_gYYshared


    grad_VCptrsArray[15] = vc_ptr + offsetof(struct grad_MemLayout, gXYshared); //vc_gXYshared


    grad_VCptrsArray[16] = vc_ptr + offsetof(struct grad_MemLayout, xtrBIP2shared); //vc_xtrBIP2shared
    grad_VCptrsArray[17] = vc_ptr + offsetof(struct grad_MemLayout, computeBIP2shared); //vc_computeBIP2shared


    grad_VCptrsArray[18] = vc_ptr + offsetof(struct grad_MemLayout, dPosXYshared); //vc_dPosXYshared


    grad_VCptrsArray[19] = vc_ptr + offsetof(struct grad_MemLayout, detXYshared); //vc_detXYshared


    grad_VCptrsArray[20] = vc_ptr + offsetof(struct grad_MemLayout, deltaXYshared); //vc_deltaXYshared
}
...
void grad_init()
{
    ...
    for (int i=0; i < NUM_QPUS; i++)
    {
        ...
        arm_map->uniforms[i][0] = grad_VCptrsArray[8]; //vc_firstFrameShared
        arm_map->uniforms[i][1] = grad_VCptrsArray[10]; //vc_featureXYshared
        arm_map->uniforms[i][2] = grad_VCptrsArray[11]; //vc_gradXshared
        arm_map->uniforms[i][3] = grad_VCptrsArray[12]; //vc_gXXshared
        arm_map->uniforms[i][4] = grad_VCptrsArray[13]; //vc_gradYshared
        arm_map->uniforms[i][5] = grad_VCptrsArray[14]; //vc_gYYshared
        arm_map->uniforms[i][6] = grad_VCptrsArray[15]; //vc_gXYshared
        arm_map->uniforms[i][7] = i; //QPU number
        arm_map->uniforms[i][8] = grad_VCptrsArray[9]; //vc_secondFrameShared
        arm_map->uniforms[i][9] = grad_VCptrsArray[16]; //vc_xtrBIP2shared
        arm_map->uniforms[i][10] = grad_VCptrsArray[18]; //vc_dPosXYshared
        arm_map->uniforms[i][11] = grad_VCptrsArray[17]; //vc_computeBIP2shared
        arm_map->uniforms[i][12] = grad_VCptrsArray[19]; //vc_detXYshared
        arm_map->uniforms[i][13] = grad_VCptrsArray[20]; //vc_deltaXYshared

        arm_map->msg[i][0] = grad_VCptrsArray[6] + i * sizeof(unsigned) * NUM_UNIS; //vc_uniforms + i*NUM_UNIS
...
void grad_featureXYsharedCpy(unsigned int *featureXYmain)
{
    ...
    memcpy(arm_map->featureXYshared, featureXYmain, sizeof(unsigned int) * 2 * NUM_FEATURES);
...
void grad_gradXqpu(void)
{
    ...
    execute_qpu(mb, NUM_QPUS, grad_VCptrsArray[7], GPU_FFT_NO_FLUSH, GPU_FFT_TIMEOUT); //vc_msg
}
...
void grad_gradXsharedCpy(float *gradXmain)
{
    ...
    memcpy(gradXmain, arm_map->gradXshared, NUM_FEATURES*GRAD_ELMTS * sizeof(float));
...
void grad_gradYsharedCpy(float *gradYmain)
{
    ...
    memcpy(gradYmain, arm_map->gradYshared, NUM_FEATURES*GRAD_ELMTS * sizeof(float));
...
void grad_detXYsharedCompute()
{
	for(int i = 0; i < NUM_FEATURES; i++)
	{
		*(arm_map->detXYshared + i) = 1/( (*(arm_map->gXXshared + i)) * (*(arm_map->gYYshared + i)) - (*(arm_map->gXYshared + i)) * (*(arm_map->gXYshared + i)) );
	}
}
...
/*######################*/
/*__deltaXY__FUNCTIONS__*/
/*######################*/

/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
/* grad_deltaXYsharedDisplay() */
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void grad_deltaXYsharedDisplay(void)
{

    /* Check pointers values */
    printf("\nDRIVER Check arm_map->deltaXYshared = %p\n", arm_map->deltaXYshared);

    /* Display deltaXYshared verbose */
    for (int i=0; i < NUM_FEATURES*2; i++) {
        printf(" DRIVER *(arm_map->deltaXYshared + %04d) = %.8ff\n", i, *(arm_map->deltaXYshared + i));
    }


    /* Display only values */
    /*for (int i=0; i < NUM_FEATURES; i++) {
        printf("%f\n", *(arm_map->deltaXYshared + i));
    }*/
...
/*~~~~~~~~~~~~~~~~~~~~~~~*/
/* grad_deltaXYsharedCpy */
/*~~~~~~~~~~~~~~~~~~~~~~~*/
void grad_deltaXYsharedCpy(float *deltaXYmain)
{

    /* Copy deltaXYshared in non-shared CPU RAM memory */
    memcpy(deltaXYmain, arm_map->deltaXYshared, 2*NUM_FEATURES*sizeof(float));


    /*__DEBUG__*/
    /*for (int i=0; i < 2*NUM_FEATURES; i++) {
        printf("DRIVER memcpy *(arm_map->deltaXYshared + %03d) = %.8ff", i, *(arm_map->deltaXYshared + i), *(arm_map->deltaXYshared + i));
        printf("-----DRIVER memcpy *(deltaXYmain+ %03d) = %.8ff\n", i, *(deltaXYmain + i), *(deltaXYmain + i));
    }*/
    /*__DEBUG__*/
}
/*###########################*/
/*__END__deltaXY__FUNCTIONS__*/
/*###########################*/
